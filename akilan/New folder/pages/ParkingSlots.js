...
const ParkingSlots = () => {
  ...
  const handleSave = () => {
    if (!form.slot_id.trim()) {
      toast({ title: 'Slot ID is required', variant: 'destructive' });
      return;
    }

    if (editing) {
      updateSlot(editing.slot_id, {
      ...
      if (slots.find(s => s.slot_id === form.slot_id)) {
        toast({ title: 'Slot ID already exists', variant: 'destructive' });
        return;
      }
      addSlot({
        ...form,
  ...
  return (
    <AdminLayout>
              ...
              <tbody>
                {filtered.map(slot => (
                  <tr key={slot.slot_id}>
                    ...
                    <td>
                      <span className={slot.slot_type === 'car' ? 'badge-car' : 'badge-bike'}>
                        {slot.slot_type}
                      </span>
                    </td>
                    <td className="capitalize">{slot.shift_type}</td>
                    <td>
                      <span className={slot.status === 'free' ? 'badge-free' : 'badge-occupied'}>
                        {slot.status}
                      </span>
                    </td>
                    <td>
                      <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(slot.slot_id)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? 'Edit Slot' : 'Add New Slot'}</DialogTitle>
          </DialogHeader>
